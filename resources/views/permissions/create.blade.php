            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    <form action="{{ route('permissions.store') }}" method="POST">
                        ...
                        {{-- <div class="mt-4">
                            <label for="guard_name" class="block font-medium text-sm text-gray-700">Guard Name</label>
                            <input type="text" name="guard_name" id="guard_name" class="mt-1 block rounded-md w-full" required />
                        </div> --}}
                        <div class="mt-4">
                            <button type="submit" class="inline-flex items-center px-4 py-2 bg-blue-700 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-blue-600 active:bg-blue-700 focus:outline-none focus:border-blue-800 focus:ring focus:ring-blue-200 disabled:opacity-25 transition">Create Permission</button>
                        </div>
                    </form>
                </div>
            </div>
